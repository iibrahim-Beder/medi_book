...
import 'package:medi_book/features/doctor_details/presentation/screen/widgets/doctor_list_view_review_item.dart';
...
class _DoctorReviewsListState extends State<DoctorReviewsList> {
  ...
  void _onScroll() {
    // prevent running this logic too often (every 200ms max)
    if (_throttleTimer?.isActive ?? false) return;

    _throttleTimer = Timer(const Duration(milliseconds: 200), () {
      // make sure the widget is still in the widget tree
      if (!mounted) return;
      ...
      // check if the last visible item is 2 items away from the end
      final isNearBottom = lastVisibleIndex >= widget.reviews.length - 2;

      // the logic of getting more data
      if (isNearBottom &&
          paginatedState.hasMoreData &&
          !paginatedState.isLoadingMore) {
        cubit.fetchMoreDoctorReviews(0);
      }

      // convert Iterable<PositionedItem> to List<PositionedItem>
  ...
  Widget build(BuildContext context) {
    ...
    return RefreshIndicator(
      ...
      onRefresh: () async {
        cubit.fetchDoctorReviews(0);
      },
      child: ScrollablePositionedList.builder(
        initialScrollIndex: cubit.state.reviewsLastindex,
        initialAlignment: cubit.state.reviewsAlignment,
